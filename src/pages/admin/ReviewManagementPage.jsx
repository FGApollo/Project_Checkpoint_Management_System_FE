...
const ReviewManagementPage = () => {
  ...
  const handleMatch = async () => {
    ...
    if (boardData.registeredLecturersCount < MIN_REVIEWERS_PER_SESSION) {
      setSuccess('');
      setError(`Cần ít nhất ${MIN_REVIEWERS_PER_SESSION} giảng viên nộp đăng ký trùng một slot trước khi chạy phân công. Hiện có ${boardData.registeredLecturersCount} giảng viên đã nộp${boardData.draftLecturersCount > 0 ? ` và ${boardData.draftLecturersCount} giảng viên mới lưu nháp` : ''}.`);
      return;
    }
    setError('');
    setSuccess('');
    ...
    try {
      ...
      const res = await api.post('/review-scheduling/random-assign', {
        ...
        reviewersPerSession: MIN_REVIEWERS_PER_SESSION,
        roomPrefix: 'REV'
      });
      const data = res.data || {};
      ...
      if (assignedCount === 0) {
        const unassignedReasons = data.unassignedGroups?.map(u => `${u.groupCode}: ${u.reason}`).join(' | ') || `Chưa đủ giảng viên đăng ký cùng một slot (cần từ ${MIN_REVIEWERS_PER_SESSION}-${MAX_REVIEWERS_PER_SLOT} giảng viên và không trùng với GVHD).`;
              ...
              <>
                ...
                {isRoundPublished(selectedRound.status) ? (
                  ...
                  <>
                    ...
                    <div style={{ background: '#F8FAFC', borderRadius: '12px', border: `1px dashed ${boardData.registeredLecturersCount >= MIN_REVIEWERS_PER_SESSION ? '#CBD5E1' : '#F59E0B'}`, padding: '2.5rem', maxWidth: '540px', margin: '0 auto 2rem' }}>
                      {boardData.registeredLecturersCount >= MIN_REVIEWERS_PER_SESSION ? <Zap size={48} color="#D97706" style={{ margin: '0 auto 1rem' }} /> : <AlertCircle size={48} color="#D97706" style={{ margin: '0 auto 1rem' }} />}
                      <h3 style={{ fontSize: '1.1rem', color: '#0F172A', marginBottom: '0.5rem' }}>{boardData.registeredLecturersCount >= MIN_REVIEWERS_PER_SESSION ? 'Sẵn sàng chạy thuật toán' : 'Chưa đủ đăng ký slot'}</h3>
                      <p style={{ fontSize: '0.85rem', color: '#64748B', marginBottom: '1.5rem', lineHeight: 1.6 }}>
                        {boardData.registeredLecturersCount >= MIN_REVIEWERS_PER_SESSION
                          ? `Hệ thống ghép toàn bộ giảng viên đã đăng ký trùng slot vào cùng hội đồng, từ ${MIN_REVIEWERS_PER_SESSION} đến tối đa ${MAX_REVIEWERS_PER_SLOT} người.`
                          : `Cần ít nhất ${MIN_REVIEWERS_PER_SESSION} giảng viên nộp đăng ký trùng một slot. Hiện có ${boardData.registeredLecturersCount} giảng viên đã nộp${boardData.draftLecturersCount > 0 ? `; ${boardData.draftLecturersCount} giảng viên mới lưu nháp` : ''}.`}
                      </p>
                      <button type="button" className="btn btn-primary" onClick={handleMatch} disabled={loading || boardData.registeredLecturersCount < MIN_REVIEWERS_PER_SESSION} style={{ padding: '0.75rem 2rem', fontSize: '1rem', background: '#4F46E5', opacity: boardData.registeredLecturersCount < MIN_REVIEWERS_PER_SESSION ? 0.5 : 1, cursor: boardData.registeredLecturersCount < MIN_REVIEWERS_PER_SESSION ? 'not-allowed' : 'pointer' }}>
                        <Play size={18} fill="white" /> {loading ? 'Đang chạy Auto-Match...' : boardData.registeredLecturersCount < MIN_REVIEWERS_PER_SESSION ? 'Chưa thể Auto-Match' : 'Chạy Auto-Match'}
                      </button>
                    </div>
                  </>
                )}
